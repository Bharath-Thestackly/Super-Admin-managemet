package com.example.common.abstracts;
 ...
 * with built-in Multi-Tenancy support via TenantContext.
 *
 * Feature services should extend this class and implement:
 *   - {@link #toEntity(Object)}          — map REQ_DTO → Entity
 *   - {@link #toDto(Object)}             — map Entity → RES_DTO
 *   - {@link #updateEntityFromDto(Object, Object)} — patch entity fields from REQ_DTO
 *
 * Lifecycle hooks (beforeCreate, afterCreate, etc.) may optionally be overridden.
 *
 * @param <T>       Entity type
...
public abstract class AbstractService<T, ID, REQ_DTO, RES_DTO> implements BaseService<T, ID, REQ_DTO, RES_DTO> {
    ...
    // ---------------------------------------------------------------
    // Abstract mapping methods — subclasses must implement these
    // ---------------------------------------------------------------
    ...
    /** Update target entity fields from incoming request DTO (used by update). */
    ...
    public RES_DTO create(REQ_DTO requestDto) {
        ...
        // Auto-inject tenantId into BaseEntity subclasses if not already set
